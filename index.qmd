---
title: "Escaping the circles of installation hell"
author:
  - name: Raniere Silva
    orcid: 0000-0002-8381-3749
    email: Raniere.CostadaSilva@gesis.org
    affiliations:
      - name: GESIS Leibniz Institute for the Social Sciences
  - name: Jannetta Steyn
    affiliations:
      - name: Newcastle University
format:
  revealjs:
    min-scale: 1
    max-scale: 1
---

## Software Carpentry 1.0

::: {.notes}
This is how Software Carpentry started.
:::

> In response, John Reynders (then director of the Advanced Computing Laboratory at Los Alamos National Laboratory) invited the [Greg Wilson] and Brent Gorda (now at Intel) to teach a week-long course to LANL staff. This course ran for the first time in July 1998, and was repeated nine times over the next four years.

Wilson G. Software Carpentry: lessons learned [version 2; peer review: 3 approved]. F1000Research 2016, 3:62 ([https://doi.org/10.12688/f1000research.3-62.v2](https://doi.org/10.12688/f1000research.3-62.v2)) 

## In July of 1998 ...

::: {.notes}
This a summary of the software landscape when Software Carpentry started.
:::

:::: {.columns}

::: {.column width="50%"}
### Python

Version [1.5.1](https://github.com/python/cpython/releases/tag/v1.5.1) has just been released.

Python Package Index (PyPI) has not been created, [it was launched in 2003](https://en.wikipedia.org/wiki/Python_Package_Index).

Numeric (now called NumPy) was [announced in 1995](http://hugunin.net/papers/hugunin95numpy.html).

<!-- SciPy has not been created, [first release around 2001](https://arxiv.org/abs/1907.10121). -->
:::

::: {.column width="50%"}
### R

Version [0.62.2](https://cran.r-project.org/src/base/R-0/) has just been released.

The Comprehensive R Archive Network [(CRAN) has just been founded in 1997](https://en.wikipedia.org/wiki/R_(programming_language)).

ggplot2 has not been created, [first release in 10 June 2007](https://en.wikipedia.org/wiki/Ggplot2).
:::

::::

## Back to the Future, or 2012

::: {.notes}
This is when Raniere got involved with Software Carpentry.
:::

> Software Carpentry rebooted again in January 2012 with a grant from the Sloan Foundation to the Mozilla Foundation. This time, the model was two-day intensive workshops like those pioneered by The Hacker Within, a grassroots group of grad students helping grad students at the University of Wisconsin - Madison.

Wilson G. Software Carpentry: lessons learned [version 2; peer review: 3 approved]. F1000Research 2016, 3:62 ([https://doi.org/10.12688/f1000research.3-62.v2](https://doi.org/10.12688/f1000research.3-62.v2)) 

## In 2012 ...

::: {.notes}
This a summary of the software landscape when Software Carpentry rebooted as part of Mozilla Science Lab.
:::

:::: {.columns}

::: {.column width="50%"}
### Python

Versions [2.7.3](https://github.com/python/cpython/releases/tag/v2.7.3) and [3.1.5](https://github.com/python/cpython/releases/tag/v3.1.5) have just been released.

Many packages from Python Package Index (PyPI) [only work with Python 2.x.y](https://en.wikipedia.org/wiki/History_of_Python#Compatibility).
:::

::: {.column width="50%"}
### R

Version [2.15.0](https://cran.r-project.org/src/base/R-2/) has jut been released.

RStudio IDE was [released in 2011](https://posit.co/blog/rstudio-new-open-source-ide-for-r/).

dplyr (and tidyverse) has not been created, [first release in 2014](https://dplyr.tidyverse.org//news/index.html#dplyr-011).
:::

::::

## In 2024 ...

::: {.notes}
This a summary of the software landscape today.
:::

:::: {.columns}

::: {.column width="50%"}
### Python

Version [3.13.0](https://github.com/python/cpython/releases/tag/v3.13.0) has just been released.
:::

::: {.column width="50%"}
### R

Version [4.4.1](https://cran.r-project.org/src/base/R-4/) has jut been released.
:::

::::